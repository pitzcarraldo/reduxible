...
export default class Reduxible {
  ...
  server() {
    return (req, res, next) => {
      ...
      const store = this.storeFactory.createStore({}, [ contextMiddleware({ server:true, ...{ req, res, next } }) ]);
  ...
  client(initialState, dest) {
    const store = this.storeFactory.createStore(initialState, [contextMiddleware({ client: true })]);
